...
ACTION polcontract::rebalance(){
    ...
    if( !lp_details.is_in_range && s.last_liquidity_addition_time < now() - ( SECONDS_PER_DAY * 7 ) ){
    	...
    	if( s.lswax_bucket >= dc3.minimum_unliquify_amount ){
    		...
    		int64_t output_amount = internal_unliquify( s.lswax_bucket.amount, ds );
    	...
    	if( s.wax_bucket > ZERO_WAX && s.lswax_bucket == ZERO_LSWAX ){
	    	...
	    	if( s.lswax_bucket >= dc3.minimum_unliquify_amount ){		
	    		...
	    		//calculate the amount of swax our lswax is worth
	    		int64_t max_output_amount = internal_unliquify( s.lswax_bucket.amount, ds );

	    		//amount to transfer is min( lswax_bucket, dapp.available_for_rentals )
	    		int64_t max_weight = std::min( max_output_amount, max_redeemable );

	    		//this needs to also factor in the real price somehow
	    		int64_t weighted_amount_to_transfer = max_weight / 2;
    		...
    		int64_t half_weight = total_weight / 2;
    	...
    	//or maybe there is? (more earning power in dapp contract)?
